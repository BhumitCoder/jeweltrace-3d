...
function BlogEditor() {
  ...
  // All localStorage access is here — only runs on the client
  useEffect(() => {
    if (!isAdminAuthed()) {
      navigate({ to: "/admin" });
      return;
        ...
        <div className="mt-6 mb-8 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1.5 text-sm">
              Write your article below. It will appear on the public blog immediately after saving.
            </p>
          </div>
          {!isNew && post.slug && (
        ...
        <form onSubmit={submit} className="space-y-6">
          ...
          <div className="p-6 rounded-2xl border border-border bg-card/60 backdrop-blur-sm shadow-elegant space-y-5">
            ...
            <div className="grid sm:grid-cols-2 gap-5">
              <Field label="URL Slug" required>
                <input value={post.slug} onChange={(e) => update("slug", slugify(e.target.value))}
              ...
              <Field label="Author">
                <input value={post.author} onChange={(e) => update("author", e.target.value)}
                  className={ic} placeholder="JewelReport Editorial" />
              </Field>
              <Field label="Published Date">
                <input type="date" value={post.publishedAt} onChange={(e) => update("publishedAt", e.target.value)} className={ic} />
              </Field>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {post.coverDataUrl && (
                    <div className="relative">
                      ...
                      <button type="button" onClick={() => update("coverDataUrl", undefined)}
          ...
          <div className="p-6 rounded-2xl border border-border bg-card/60 backdrop-blur-sm shadow-elegant">
            ...
            <textarea
              ...
              placeholder="Write your article here. Blank lines become paragraph breaks."
              required
            />
            <p className="text-xs text-muted-foreground mt-2">Plain text only — blank lines separate paragraphs.</p>
          </div>
